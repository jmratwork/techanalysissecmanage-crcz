...
start_bips() {
    ...
    if systemctl start bips >>/var/log/bips/service.log 2>&1; then
        ...
        nc -z localhost "${BIPS_PORT}" >>/var/log/bips/service.log 2>&1 || {
...
start_cicms() {
    ...
    if systemctl start cicms >>/var/log/cicms/service.log 2>&1; then
        ...
        nc -z localhost "${CICMS_PORT}" >>/var/log/cicms/service.log 2>&1 || {
...
start_ng_soc() {
    ...
    if systemctl start ng-soc >>/var/log/ng_soc/service.log 2>&1; then
        ...
        nc -z localhost "${NG_SOC_PORT}" >>/var/log/ng_soc/service.log 2>&1 || {
